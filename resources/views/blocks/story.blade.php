<div id="fh5co-couple-story" data-scroll-top="chuyen-tinh">
    <div class="container">
        <div class="row">
            <div class="col-md-8 col-md-offset-2 text-center fh5co-heading animate-box">
                <!-- <span>We Love Each Other</span> -->
                <h2><a href="/chuyen-tinh">Chuyện Tình</a></h2>
                <p class="poem-love">
                    Tâm bất biến giữa dòng đời vạn biến</br>
                    Ta sẽ cùng nhau lấp biển, rời non
                </p>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12 col-md-offset-0">
                <ul class="timeline animate-box">
		    <li class="animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/ky-yeu-cap-3.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Lời yêu đầu</h3>
                                <span class="date">05/08/2012</span>
                            </div>
                            <div class="timeline-body">
                                <p>Tôi từng nghe nói rằng "Cô gái đi bên bạn năm 17 tuổi sẽ không cùng bạn đi hết chặng đường đời". Vì thế tôi đã đưa ra một quyết định vô cùng đột phá và táo bạo, tránh cái dớp 17.<br/>Nói yêu cô ấy năm 15 tuổi.</p>
                            </div>
                        </div>
                    </li>
                    <li class="timeline-inverted animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/tinh-yeu-bo-xit.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Tình yêu bọ xít</h3>
                                <span class="date">22/05/2013</span>
                            </div>
                            <div class="timeline-body">
                                <p>Cái thời còn nhắn tin hàng ngày cho nhau bằng chiếc điện thoại Nokia "cục gạch". 100đ/1 tin nhắn trao gửi yêu thương :))</p>
                            </div>
                        </div>
                    </li>
                    <li class="animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/con-doc-ky-niem.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Con dốc kỷ niệm</h3>
                                <span class="date">12/09/2014</span>
                            </div>
                            <div class="timeline-body">
                                <p>Đón đưa nhau đi học hàng ngày qua con dốc dài, con dốc chất chứa những khoảnh khắc êm đềm của cả một hành trình tình yêu của hai đứa.</p>
                            </div>
                        </div>
                    </li>
                    <li class="timeline-inverted animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/ky-yeu-cap-3.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Kỷ yếu cấp 3</h3>
                                <span class="date">25/05/2015</span>
                            </div>
                            <div class="timeline-body">
                                <p>Cùng nhau trải qua những khoảnh khắc quan trọng nhất cuộc đời học sinh.</p>
                            </div>
                        </div>
                    </li>
                    <li class="animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/ngay-dau-len-ha-noi.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Những ngày đầu lên Thủ đô</h3>
                                <span class="date">05/09/2015</span>
                            </div>
                            <div class="timeline-body">
                                <p>Những ngày đầu bỡ ngỡ khi mới lên Hà Nội. Hai đứa đầy hứng khởi đi thăm thú khắp nơi, đưa nhau qua những con phố lớn nhỏ.</p>
                            </div>
                        </div>
                    </li>
                    <li class="timeline-inverted animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/du-lich-ba-vi.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Phượt Ba Vì</h3>
                                <span class="date">27/02/2016</span>
                            </div>
                            <div class="timeline-body">
                                <p>Lần đầu đi phượt xe máy cùng với anh chị. Tự tay chuẩn bị đồ ăn, vật dụng cắm trại. Ba Vì vào xuân thật đẹp!</p>
                            </div>
                        </div>
                    </li>
                    <li class="animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/quan-su.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Kỳ học quân sự</h3>
                                <span class="date">09/04/2016</span>
                            </div>
                            <div class="timeline-body">
                                <p>Chuyến đi thăm nuôi "cô dân quân" hơi nhiều sóng gió :v Nhờ đó mà mình nhận ra có thể vượt qua mọi khó khăn để được ở gần nhau.</p>
                            </div>
                        </div>
                    </li>
		    <li class="timeline-inverted animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/ho-tay.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Một thoáng Hồ Tây</h3>
                                <span class="date">11/06/2017</span>
                            </div>
                            <div class="timeline-body">
                                <p>Hồ Tây là địa điểm lượn lờ thường xuyên của 2 đứa, cùng nhau lượn hết vòng hồ tây, cùng kể cho nhau nghe những điều thú vị trong cuộc sống.</p>
                            </div>
                        </div>
                    </li>
                    <li class="animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/co-vu-bong-da.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Cổ vũ bóng đá</h3>
                                <span class="date">27/01/2018</span>
                            </div>
                            <div class="timeline-body">
                                <p>2018 - Một năm khó quên của nền bóng đá nước nhà, cùng nhau hòa mình vào không khí tuyệt vời đó. Việt Nam vô địch!</p>
                            </div>
                        </div>
                    </li>
                    <li class="timeline-inverted animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/ky-yeu-dai-hoc.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Kỷ yếu Đại học</h3>
                                <span class="date">28/04/2019</span>
                            </div>
                            <div class="timeline-body">
                                <p>Một lần nữa đồng hành cùng nhau trong cột mốc tiếp theo của cuộc đời</p>
                            </div>
                        </div>
                    </li>
                    <li class="animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/du-lich-ha-long.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Hạ Long cùng nhóm bạn thân</h3>
                                <span class="date">18/07/2020</span>
                            </div>
                            <div class="timeline-body">
                                <p>Cùng với hội bạn thân đi du lịch. Chuyến đi hơi ngắn ngày nhưng chúng ta đã có những kỷ niệm tuyệt vời và khó quên bên cạnh bạn bè.</p>
                            </div>
                        </div>
                    </li>
		    <li class="timeline-inverted animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/sinh-nhat.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Sinh nhật</h3>
                                <span class="date">26/10/2021</span>
                            </div>
                            <div class="timeline-body">
                                <p>Cùng nhau trưởng thành, trải qua ngày sinh nhật đặc biệt và khó quên.</p>
                            </div>
                        </div>
                    </li>
		    <li class="animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/hen-ho.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Hẹn hò</h3>
                                <span class="date">10/04/2022</span>
                            </div>
                            <div class="timeline-body">
                                <p>Sau những guồng xoáy của công việc, vẫn dành cho nhau thời gian vào những buổi hẹn hò đơn giản mà hạnh phúc.</p>
                            </div>
                        </div>
                    </li>
                    <li class="timeline-inverted animate-box">
                        <div class="timeline-badge lazy" data-src="images/story/du-lich-da-nang.jpg"></div>
                        <div class="timeline-panel">
                            <div class="timeline-heading">
                                <h3 class="timeline-title">Du lịch Huế - Đà Nẵng - Hội An</h3>
                                <span class="date">16/07/2022</span>
                            </div>
                            <div class="timeline-body">
                                <p>Chuyến bay đầu tiên cùng nhau của 2 đứa. Cách ngày bay một tuần cả 2 đều bị Covid nhưng thật may đã kịp khỏi và chúng ta đã có một chuyến đi trọn vẹn, nhẹ nhàng và đầy ý nghĩa.</p>
                            </div>
                        </div>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</div>